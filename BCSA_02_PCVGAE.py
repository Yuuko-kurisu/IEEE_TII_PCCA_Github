...
"""
BCSA_02_PCVGAE.py
BCSA假设条件化PC-VGAE模块（重构版 - 真实数据处理）
负责：基于假设进行条件化训练和不确定性聚合
职责：单一职责 - 假设条件化的审计分析
"""
...
class HypothesisConditionedPCVGAE(nn.Module):
    """假设条件化PC-VGAE模型 - """
    ...
    def __init__(self, node_features_dim: int, hypothesis_feature_dim: int = 16, 
        ...
        logger.info(f"假设条件化PC-VGAE初始化: 节点特征{node_features_dim}维, "
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        h2 = self.encoder2(h1, edge_index)
...
class HypothesisConditionedUncertaintyAnalyzer:
    """假设条件化不确定性分析器 - 重构版"""
    ...
    def __init__(self, prompt_dim: int = 64, device: str = 'cpu', enable_hypothesis_fasttrack: bool = True):
        ...
        self.learning_rate = 5e-4  # 降低学习率
        self.max_epochs = 800      # 大幅增加训练周期
        self.kl_weight = 0.1

        logger.info(f"假设条件化不确定性分析器初始化完成，直升通道: {'启用' if enable_hypothesis_fasttrack else '禁用'}")
    ...
    def quantify_impact_and_aggregate(self, high_quality_hypotheses: List[Hypothesis],
                                    ...
                                    idx_to_node: Dict[int, str], data_context: Dict[str, Any] = None,
                                    scoring_params: Optional[Dict[str, float]] = None): 
        """量化假设影响力并进行加权聚合（多次推理）- 增强版 + 直升通道"""
        logger.info(f"开始量化 {len(high_quality_hypotheses)} 个高质量假设的影响力...")
        
        if not self.torch_available or self.unified_model is None:
            logger.error("模型未准备好，无法进行推理")
            return {}
        ...
        if not remaining_hypotheses:
            logger.info("所有假设都已通过直升通道处理，跳过常规PC-VGAE流程")
            return {
        ...
        logger.info(f"对剩余 {len(remaining_hypotheses)} 个假设进行常规PC-VGAE分析...")
        
        logger.info("计算基准不确定性地图...")
        ...
        with torch.no_grad():
            ...
            mu_base, logvar_base, z_base = self.unified_model(X_baseline, edge_index)
        ...
        for hyp_idx, hypothesis in enumerate(sorted_remaining_hypotheses):
            ...
            with torch.no_grad():
                mu_cond, logvar_cond, z_cond = self.unified_model(X_conditioned, edge_index)
            ...
            kl_divergence = np.sum(
                conditioned_map * np.log(conditioned_map / (self.baseline_map + epsilon) + epsilon)
            )

            l2_norm_impact = np.linalg.norm(delta_map)
            total_impact = l2_norm_impact + kl_divergence * 0.5 # KL散度作为加权项
        ...
        for edge_key in sorted(candidate_pool.keys()):
            ...
            final_score = self.calculate_final_finding_score(
                ...
                scoring_params=scoring_params # <-- 关键传递
            )
    ...
    def train_unified_model(self, graph_data: Any, node_features: Dict[str, np.ndarray],
                        negative_edges: Any, unified_features: Dict[str, np.ndarray],
                        training_params_override: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """训练统一的PC-VGAE模型（一次训练）- """
        logger.info("开始训练统一PC-VGAE模型...")
        ...
        if training_params_override and 'kl_weight' in training_params_override:
            kl_weight = training_params_override['kl_weight']
            logger.info(f"使用覆盖的 kl_weight: {kl_weight}")
        ...
        self.unified_model = HypothesisConditionedPCVGAE(
        ...
        for epoch in range(self.max_epochs):
            ...
            mu, logvar, z = self.unified_model(x, edge_index)
            ...
            total_loss = recon_loss + kl_weight * kl_loss # <-- 使用局部变量 kl_weight
        ...
        if os.path.exists(best_model_path):
            ...
            logger.info(f"已加载最佳模型，最终损失: {best_loss:.4f}")
            os.remove(best_model_path)
        
        converged = patience_counter >= patience or (len(losses) > 20 and abs(losses[-1] - losses[-20]) < 0.001)
        
        logger.info(f"统一模型训练完成，最终损失: {best_loss:.4f}，是否收敛: {converged}")
    ...
    def _train_single_model(self, model: HypothesisConditionedPCVGAE,
                           hypothesis_prompt: HypothesisPrompt,
                           graph_data: Any, negative_edges: Any) -> Dict[str, Any]:
        """训练单个条件化模型"""
        model.train()
        ...
        edge_index = graph_data.edge_index.to(self.device)
        prompt_tensor = torch.FloatTensor(hypothesis_prompt.combined_prompt).to(self.device)
    ...
    def generate_enhanced_candidate_pool(self, hypothesis_impacts: Dict[str, Any], 
                                    conflict_scores: Dict[Tuple[str, str], float],
                                    top_k_hypotheses: int = 10, 
                                    top_n_per_hypothesis: int = 5,
                                    top_m_conflicts: int = 20) -> Dict[Tuple[str, str], Dict[str, Any]]:
        """生成增强的发现候选池"""
        logger.info("生成增强的发现候选池...")
        
        candidate_pool = {}
        
        logger.info(f"从Top-{top_k_hypotheses}假设中提取边...")
        sorted_hypotheses = sorted(
        ...
        for hyp_id, impact_data in sorted_hypotheses:
            delta_map = impact_data['delta_map']
            edge_impacts = [(i, abs(delta_map[i])) for i in range(len(delta_map))]
            top_edges = sorted(edge_impacts, key=lambda x: x[1], reverse=True)[:top_n_per_hypothesis]
            
            for edge_idx, impact_value in top_edges:
                pass
        
        logger.info(f"选择Top-{top_m_conflicts}冲突边...")
        sorted_conflicts = sorted(
            conflict_scores.items(),
            key=lambda x: x[1],
            reverse=True
        ...
        for edge_key, conflict_score in sorted_conflicts:
            ...
            candidate_pool[edge_key]['sources'].append('direct_conflict')
            candidate_pool[edge_key]['conflict_score'] = conflict_score
        
        logger.info(f"生成候选池：{len(candidate_pool)} 条边")
    ...
    def calculate_final_finding_score(self, edge_key: Tuple[str, str], 
        ...
        default_w_impact = 0.6
        default_w_conflict = 0.4

        if scoring_params:
            w_impact = scoring_params.get('alpha', default_w_impact)
            w_conflict = scoring_params.get('beta', default_w_conflict)
        else:
            w_impact = default_w_impact
            w_conflict = default_w_conflict
...
def run_conditioned_uncertainty_analysis(hypotheses: List[Hypothesis],
                                        case_dir: Path,
                                        output_dir: Path,
    ...
    """
    运行假设条件化不确定性分析的主函数 - 重构版 + 直升通道

    Args:
        hypotheses: 假设列表
        case_dir: 案例目录路径
        output_dir: 输出目录路径
        aggregation_method: 聚合方法（现在默认为weighted_impact）
        mc_samples: 蒙特卡洛采样次数
    ...
    Returns:
        聚合后的不确定性分析结果
    """
    logger.info(f"开始假设条件化不确定性分析（重构版+直升通道）: {case_dir.name}, "
    ...
    try:
        ...
        analyzer = HypothesisConditionedUncertaintyAnalyzer(enable_hypothesis_fasttrack=enable_hypothesis_fasttrack)
        
        logger.info("步骤1: 训练统一PC-VGAE模型")
        ...
        logger.info("步骤2: 量化假设影响力并进行加权聚合（含直升通道）")
        ...
        result = AggregatedUncertaintyResult(
            ...
            training_results={'unified_model': _convert_training_result(training_result, 'unified')},
            aggregation_method='weighted_impact_with_fasttrack' if enable_hypothesis_fasttrack else 'weighted_max',
        ...
        logger.info(f"假设条件化不确定性分析完成（重构版）: "
        ...
        logger.error(f"假设条件化不确定性分析失败: {e}")
        raise
...
def _identify_high_priority_findings(uncertainty_map: List[EdgeUncertaintyResult]) -> List[EdgeUncertaintyResult]:
    """识别高优先级发现"""
    if not uncertainty_map:
        return []
    
    sorted_findings = sorted(uncertainty_map, key=lambda x: abs(x.unified_score), reverse=True)
    
    high_priority_count = max(5, len(sorted_findings) // 4)
    high_priority_findings = sorted_findings[:high_priority_count]
    
    high_priority_findings = [f for f in high_priority_findings if abs(f.unified_score) > 0.5]
...
def _save_conditioned_analysis_results(result: AggregatedUncertaintyResult, output_dir: Path):
    """保存假设条件化分析结果"""
    uncertainty_map_data = []
    for edge_result in result.aggregated_uncertainty_map:
        edge_data = {
    ...
    training_summary = {
        ...
        'total_models_trained': len(result.training_results),
        'converged_models': len([r for r in result.training_results.values() if r.converged]),
        'average_final_loss': np.mean([r.final_loss for r in result.training_results.values()]) if result.training_results else 0,
        ...
        'model_details': {
            ...
            for model_id, training_result in result.training_results.items()
        }
    }
    
    training_file = output_dir / "training_summary.json"
    with open(training_file, 'w', encoding='utf-8') as f:
        json.dump(convert_numpy_types(training_summary), f, indent=2, ensure_ascii=False)
    
    logger.info(f"假设条件化分析结果已保存:")
    logger.info(f"  - 聚合不确定性地图: {uncertainty_file}")
    logger.info(f"  - 高优先级发现: {priority_file}")
    logger.info(f"  - 训练摘要: {training_file}")


def main():
    """主函数 - 独立运行假设条件化分析模块"""
    print("=" * 60)
    print("🎯 BCSA假设条件化PC-VGAE模块 - 独立运行")
    print("=" * 60)
    ...
    output_dir = Path("./results/conditioned_uncertainty_analysis")
    ...
    try:
        ...
        result = run_conditioned_uncertainty_analysis(
        ...
        print(f"\n💾 输出目录: {output_dir}")
        print(f"   - aggregated_uncertainty_map.json")
        print(f"   - final_aggregated_uncertainty_map.json")
